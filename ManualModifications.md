...
Runs and outputs help without modifications.  Trying to run configure causes issues, modify to create base config file if none exists.  Then issues with questions, the remember me clause is an issue, resolve by making it into a `.text` type.
...
async def configure_profile(profile_name: str):
    ...
    questions = [
        ...
        inquirer.Text("remember_me", "Stay logged in: skip authentication while refreshing AWS credentials", default="False"),
...
def get_aws_config_path():
    """Returns the path to the AWS config file. Creates defaults if it doesn't exist."""
    config_path = os.environ.get('AWS_CONFIG_FILE', os.path.join(os.path.expanduser('~'), '.aws', 'config'))
